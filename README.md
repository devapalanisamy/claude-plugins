...
- **dev-loop** — hybrid implement loop: the main agent implements (against current official docs) + runs live e2e (and can pause for a login), then hands the diff to a background Workflow that runs review → **consensus + cross-model adjudication** → fix → re-verify with fresh reviewers until clean; the main agent re-verifies live. Reviewers flag API usage that contradicts the official docs. Ships an opt-in `PostToolUse` lint gate (enable per repo via a `.loopcraft.json` with a `lintCommand`). Project-agnostic. Args: `--effort low|medium|high|max`, `--max-loops N`.
