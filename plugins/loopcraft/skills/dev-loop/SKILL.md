...
**Isolation for parallel runs:** also note how the project stands up its *live* verification environment and whether it is **shared by default** — a single dev/test backend, one local emulator/simulator — because concurrent dev-loop sessions must NOT share either (they corrupt each other's data and UI). Record the project's mechanism for making the backend and the device **session-unique** (e.g. a named/ephemeral backend env keyed by an identifier, a per-session device instance addressed by UDID/serial). You will use it in Step 2.
...
**Isolate per session (parallel-safe) — set this up BEFORE verifying.** Concurrent dev-loop sessions must never share a backend env or a device, or they overwrite each other's data and fight over the same UI. Make this run session-scoped:
- **Session key:** derive a stable, collision-free key from the git branch/worktree name — it is already unique per parallel session and stays stable across re-runs of the same session (so you reuse your own env rather than spawning a new one each round). Sanitize it for use in resource names.
- **Backend:** bring up a session-scoped ephemeral backend keyed by it instead of reusing a shared default (e.g. Amplify `npx ampx sandbox --identifier <key>`), and write its outputs to the path the clients read. Never point live verify at a deployed/shared environment.
- **Device:** boot or allocate a *dedicated* simulator/emulator instance for this session and target EVERY UI command (launch/tap/screenshot) at that instance's id (UDID/serial) — never "the default booted device," which another session may own.
- **One backend for all clients:** wire every client you exercise — web AND mobile — at the SAME session backend outputs, so a cross-surface flow hits one consistent environment.
- **Cleanup:** tear down the session backend/device on exit, or leave them clearly namespaced by the key; never reuse another session's env or device.
...
Because the Workflow runs in this same working tree, it inherits the session-scoped backend you stood up in Step 2 (its outputs file / env vars) — so any automated integration tests in `verifyCommands` also run against your isolated env, not a shared one. There is no device to pass it (it does no live UI e2e).
